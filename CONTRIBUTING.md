...
- **Allocation-class convention for `<new>` results** — picking the right `class="..."` depends on the API kind. The asymmetric rule is documented in the header comment at the top of `tensorflow.xml`; the short version: function-typed APIs that return a tensor (e.g., `tf.sigmoid`) use canonical `Ltensorflow/python/framework/ops/Tensor`; object-typed APIs and callback-bearing wrappers (e.g., `Estimator`, `Variable`, `train`) use a per-op class because class identity is load-bearing for virtual dispatch. See wala/ML#459 (the migration that established this) and wala/ML#465 (the convention's documentation).
