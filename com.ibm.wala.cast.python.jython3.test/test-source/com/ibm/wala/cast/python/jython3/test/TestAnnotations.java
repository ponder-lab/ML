...
public class TestAnnotations extends TestJythonCallGraphShape {
  ...
  public void testAnnotation1()
    ...
    AbstractAnalysisEngine<InstanceKey, PythonSSAPropagationCallGraphBuilder, ?> bb =
        makeEngine("annotations1.py");
  ...
  private MethodReference parametrize =
      MethodReference.findOrCreate(
          TypeReference.findOrCreate(PythonTypes.pythonLoader, "Lpytest/class/parametrize"),
  ...
  public void testAnnotation2()
      throws ClassHierarchyException, IllegalArgumentException, CancelException, IOException {
    PythonAnalysisEngine<?> bb =
        makeEngine(
            new PytestAnalysisEngine<Void>(), "annotations2.py", "pandas_shim.py", "np_shim.py");
    ...
    CG.getNodes(parametrize)
        .forEach(
            p -> {
              CG.getPredNodes(p)
                  .forEachRemaining(
                      n -> {
                        CG.getPossibleSites(n, p)
                            .forEachRemaining(
                                s -> {
                                    ...
                                    for (int i = 0; i < inst.getNumberOfUses(); i++) {
                                      PointerKey param =
                                          builder
                                              .getPointerAnalysis()
                                              .getHeapModel()
                                              .getPointerKeyForLocal(n, inst.getUse(i));
                                      System.err.println(
                                          builder.getPointerAnalysis().getPointsToSet(param));
                                    }
                                  }
                                });
                      });
