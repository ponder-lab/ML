...
public class TurtleSummary {
  public static final TypeReference turtleClassRef =
      TypeReference.findOrCreate(PythonTypes.pythonLoader, "Lturtle");
  public static final MethodReference turtleMethodRef =
      MethodReference.findOrCreate(
          turtleClassRef, Atom.findOrCreateUnicodeAtom("turtle"), AstMethodReference.fnDesc);
  public static final FieldReference turtleFieldRef =
      FieldReference.findOrCreate(
          turtleClassRef, Atom.findOrCreateUnicodeAtom("turtle"), PythonTypes.Root);

  public static final MethodReference turtleCallbackMethodRef =
      MethodReference.findOrCreate(
          turtleClassRef, Atom.findOrCreateUnicodeAtom("callback"), AstMethodReference.fnDesc);

  private final IClassHierarchy cha;

  private IMethod turtleMethod =
      new IMethod() {
  ...
  public TurtleSummary(IClassHierarchy cha) {
    ...
    BypassSyntheticClassLoader ldr =
        (BypassSyntheticClassLoader) cha.getLoader(cha.getScope().getSyntheticLoader());
  ...
  private class PythonMethodTurtleTargetSelector implements MethodTargetSelector {
    ...
    public IMethod getCalleeTarget(CGNode caller, CallSiteReference site, IClass receiver) {
      if (site.getDeclaredTarget().equals(turtleCallbackMethodRef)) {
        if (caller
            .getClassHierarchy()
            .isSubclassOf(receiver, caller.getClassHierarchy().lookupClass(PythonTypes.CodeBody))) {
          return receiver.getMethod(AstMethodReference.fnSelector);
        } else {
          return null;
        }
      } else if (receiver == null
          ? site.getDeclaredTarget().getDeclaringClass().equals(turtleClassRef)
          : receiver.equals(turtleClass)) {
        return code;
      } else {
    ...
    return new Entrypoint(fun) {
          ...
          if (x.length == 1 && x[0].equals(turtleClassRef)) {
            m.statements.add(
                insts.PutInstruction(
                    m.statements.size(), paramValues[j], paramValues[j], turtleFieldRef));
          }
        }
      ...
      public TypeReference[] getParameterTypes(int i) {
        return new TypeReference[] {
          i == 0 ? fun.getDeclaringClass().getReference() : turtleClassRef
        };
      }
