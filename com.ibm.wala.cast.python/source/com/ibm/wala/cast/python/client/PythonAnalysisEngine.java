  ...
  protected void addSummaryBypassLogic(AnalysisOptions options, String summary) {
    ...
    for (MethodReference mr : new ArrayList<>(summaries.keySet())) {
      if (mr.getName().toString().equals("do")) {
        ...
        if (stmtCount <= 2 && instanceType != null && instanceType.equals(mr.getDeclaringClass())) {
          isAutoGenerated = true;
        }

        if (isAutoGenerated && hasMethods) {
          // Remove auto-generated 'do' methods for classes with methods,
          // so PythonConstructorTargetSelector can take over.
          summaries.remove(mr);
        } else if (instanceType != null
            && classToFunDoRefs.containsKey(instanceType)
            && !classToFunDoRefs.get(instanceType).isEmpty()) {
          // It's a manual constructor allocating a class with methods, rewrite it to inject
          // trampolines
