...
- **Producer registration** — ops whose XML allocates a generic `Ltensorflow/python/framework/ops/Tensor` are resolved producer-side through `TensorGenerator.createManualGenerator`'s registry; an op missing there dead-ends delegation for every value it feeds (the `tf.multiply`/`tf.transpose` gap found during wala/ML#718). New generic-alloc ops must be registered.
