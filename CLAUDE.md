...
- The **PA** provides the assignment graph (assignment edges between variables) and points-to sets (PTS). However, PTS for tensor variables is **often empty even when every API in the call chain is modeled**. The cause is synthetic-method-return PTS-loss: the implicit PointerKey representing a synthetic call's return-value doesn't reliably propagate the fresh allocation in its body to downstream consumers. This affects both `<method name="do">` bodies and `<method name="read_data">` bodies even when both contain `<new def="x" .../><return value="x"/>`. Concrete examples observed empirically (2026-05-26 probe, 14/14 empty-PTS tensor variables across 12 representative tests): `tf.Variable.do()`'s `initial_value` formal receiving an upstream `tf.random.truncated_normal` result (both APIs modeled with `<new>`); dataset-iteration `element` values.
- **`read_data`/`read_dataset` are syntactic seeding markers, not allocation markers.** Their effect on the analysis is that the method *name* triggers `TensorTypeAnalysis` to seed the call's return-value with a tensor state, regardless of whether the body produces a `<new>` allocation. Several `read_data` methods in `tensorflow.xml` DO have `<new>+<return>` bodies (e.g., `Tensor.read_data` at `tensorflow.xml:1488`, `truncated_normal.read_data` at `:1579`); the syntactic marker is independent of body content.
- **`TensorTypeAnalysis`** is a parallel type system that uses the PA's assignment graph as its flow graph but seeds tensor types independently via the `read_data`/`read_dataset` markers above. Variables with empty PTS can still—and routinely do—have tensor types in this analysis.

When debugging why a tensor variable isn't recognized, check the `TensorTypeAnalysis` seeding path (`getDataflowSources`, `processInstruction`, `definesTensorIterable`), not just the PTS. The two failure modes are independent: an empty PTS means the PA didn't connect the variable to an allocation site (typically synthetic-method-return PTS-loss), while an empty `TensorTypeAnalysis` state means the syntactic seeding markers didn't reach the variable. Either alone is enough to make the variable invisible to downstream analysis.
