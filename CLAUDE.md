...
- **`TensorTypeAnalysis`** is a parallel type system that uses the PA's assignment graph as its flow graph but seeds tensor types independently. The current seeding criterion is method-name-based: allocations inside methods named `do`/`read_data`/`read_dataset` are recognized as tensor sources. The `read_data`/`read_dataset` markers are legacy—wala/ML#380 tracks inlining them away, and wala/ML#378 (closed but unimplemented) tracks broadening the criterion to allocation-type-based. Variables with empty PTS can still have tensor types in this analysis.
