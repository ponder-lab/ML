...
public class TestSlice extends TestJythonCallGraphShape {
  ...
  private static SSAAbstractInvokeInstruction find(
    ...
    for (SSAInstruction inst : ir.getInstructions()) {
      if (inst instanceof SSAAbstractInvokeInstruction
          && filter.apply((SSAAbstractInvokeInstruction) inst)) {
  ...
  public void testSlice1()
    ...
    SSAAbstractInvokeInstruction sliceImport =
        find(
            script,
            (SSAAbstractInvokeInstruction inst) -> {
              if (inst.getNumberOfUses() > 0) {
                ...
                if (def instanceof SSANewInstruction) {
                  return "Lwala/builtin/slice"
                      .equals(((SSANewInstruction) def).getConcreteType().getName().toString());
                }
              }
