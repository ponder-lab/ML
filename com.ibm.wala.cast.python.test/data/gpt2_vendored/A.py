...
class Gpt2(tf.keras.Model):
    ...
    def __init__(
        ...
        self.embedding = EmbeddingLayer(self.vocab_size, self.d_model)

        self.pos_embedding = PositionEmbeddingLayer(self.max_seq_len, self.d_model)

        self.decoder_layers = [
            DecoderLayer(self.d_model, self.num_heads, self.dff)
    ...
    def call(self, x, training=True, past=None):
        ...
        for decoder_layer, past in zip(self.decoder_layers, pasts):
            hidden_states, present = decoder_layer(
                hidden_states, training, att_mask, past=past
            )
    ...
    def get_padded_accuracy(labels, logits):
        with tf.name_scope("padded_accuracy"):
            ...
            nonpad_seq = tf.math.count_nonzero(
                weights,
    ...
    def create_checkpoint_manager(
        self, checkpoint_path, max_to_keep=5, load_model=True
    ):
    ...
    def _distributed_train_step(self, inputs, targets):
        ...
        per_example_losses = self.mirrored_strategy.run(step_fn, args=(inputs, targets))
    ...
    def fit(self, train_dataset, graph_mode):
        ...
        if self.mirrored_strategy is None:
                ...
                if step == 0:
                    with self.train_writer.as_default():
                        tf.summary.trace_export(
                            name="gpt-2", step=0, profiler_outdir=LOG_DIR
                        )

                if step % 500 == 0:
                    losses = []
                    perplexities = []
                    for test_step, (test_inputs, test_targets) in enumerate(
                        test_dataset
                    ):
                        test_loss, test_perplexity = test_func(
            ...
            with self.mirrored_strategy.scope():
                train_dataset, test_dataset = train_dataset
                train_func, test_func = self.get_distributed_train_test_function(
                    graph_mode
                )
                tf.summary.trace_on(graph=True, profiler=False)
                for step, (inputs, targets) in enumerate(train_dataset):
                    ...
                    if step % 100 == 0:
                        self.log_summary(self.train_writer, step, loss, perplexity)
    ...
    def log_summary(tf_writer, step, loss, perplexity, result_type="Train"):
        ...
        print(
            result_type
            + ":- Step {}, Loss {:.4f}, Perplexity {:.4f}".format(
...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, dr_rate=0.1):
    ...
    def call(self, x, training, mask, past=None):
        out, present = self.mha(
            self.layer_norm1(x), mask=mask, past_layer=past, training=training
        )  # (batch_size, input_seq_len, d_model)
        with tf.name_scope("residual_conn"):
            x = x + out
        out = self.feed_forward(
            self.layer_norm2(x), training=training
        )  # (batch_size, input_seq_len, d_model)
        with tf.name_scope("residual_conn"):
            x = x + out
        return x, present
...
_train = input_fn("dummy.tfrecord")
_test = input_fn("dummy.tfrecord")
_model = Gpt2(
    num_layers=1, d_model=8, num_heads=2, dff=16, max_seq_len=4, vocab_size=10
)
_model.fit([_train, _test], False)
