...
class EmbeddingLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, vocab_size, embedding_size, initializer=None, stddev=0.01, mean=0.0
    ):
...
class PositionEmbeddingLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        self, position_seq, pos_embedding_size, trainable=True, stddev=0.02, mean=0.0
    ):
        ...
        if trainable:
            self.position_embedding = EmbeddingLayer(
                self.position_seq, self.hidden_size, stddev=self.stddev, mean=self.mean
            )
        ...
        with tf.name_scope("pos_embedding"):
            if self.trainable:
                ...
                positions = tf.reshape(
                    tf.tile(tf.range(start, batch_seq + start), [batch_size]),
    ...
    @staticmethod
    def get_position_sinusoid(
        seq_len, hidden_size, min_timescale=1.0, max_timescale=1.0e4
    ):
