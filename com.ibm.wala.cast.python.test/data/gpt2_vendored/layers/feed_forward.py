...
class Conv1d(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.weight = self.add_weight(
            ...
            initializer=tf.random_normal_initializer(
                stddev=self.weights_init_stdev, mean=self.weights_mean
            ),
        )
    ...
    def call(self, inputs):
        ...
        inputs = tf.reshape(
            inputs, [-1, self.hidden_size]
        )  # shape [batch, seq , features] => [batch*seq, features]
        outputs = tf.matmul(inputs, self.weight) + self.bias
        outputs = tf.reshape(
            outputs, output_shape
        )  # Reshape => [batch, seq, filter_size]
        return outputs
...
class FeedForward(tf.keras.layers.Layer):
    ...
    def __init__(
        self, hidden_size, filter_size, dropout_rate=0.1, activation=tf.nn.relu
    ):
    ...
    def call(self, x, training=False):
        ...
        if training:
            output = tf.nn.dropout(
                output, rate=self.dropout_rate, name="feed_forward_dropout"
            )
