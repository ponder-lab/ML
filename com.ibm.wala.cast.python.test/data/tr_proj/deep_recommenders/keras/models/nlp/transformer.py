    ...
    def call(self, inputs, **kwargs):
        ...
        for pos in range(seq_length):
            for i in range(self._model_dim):
                position_encodings[pos, i] = pos / np.power(
                    10000, (i - i % 2) / self._model_dim
                )
...
class Noam(Callback):
    ...
    def on_batch_end(self, epoch, logs=None):
        logs = logs or {}
        self._step_num += 1
        lrate = self._model_dim**-0.5 * K.minimum(
            self._step_num**-0.5, self._step_num * self._warmup_steps**-1.5
        )
