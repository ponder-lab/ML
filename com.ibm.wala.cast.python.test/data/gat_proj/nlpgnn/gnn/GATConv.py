...
class GraphAttentionConvolution(MessagePassing):
    ...
    def message_function(
        ...
        """
        # 计算注意力系数
        edge_source_states = tf.reshape(
            edge_source_states, [-1, self.heads, self.out_features]
        )  # [M,Head,dim]
        edge_target_states = tf.reshape(
            edge_target_states, [-1, self.heads, self.out_features]
        )  # [M,Head,dim]
        # self.att=[1,heads,2*D]
        # [M,keads,2D] * [1,heads,2D]
        alpha = (
            tf.concat([edge_target_states, edge_source_states], -1) * self.att
        )  # [M,heads,2D]
        alpha = tf.reduce_mean(alpha, -1)
        alpha = tf.nn.leaky_relu(alpha, self.negative_slope)
        alpha = masksoftmax(
            alpha, edge_target
        )  # here not provide nodes num, because we have add self loop at the beginning.
