...
class BERT_NER(tf.keras.Model):
    ...
    def call(self, inputs, is_training=True):
        ...
        # 损失计算
        log_likelihood, transition = self.crf(
            predict, Y, sequence_lengths=tf.reduce_sum(input_mask, 1)
        )
        loss = tf.math.reduce_mean(-log_likelihood)
        predict, viterbi_score = self.crf.crf_decode(
            predict, transition, sequence_length=tf.reduce_sum(input_mask, 1)
        )
        return loss, predict
...
manager = tf.train.CheckpointManager(
    checkpoint, directory="./save", checkpoint_name="model.ckpt", max_to_keep=3
)
# For train model
Batch = 0
for X, token_type_id, input_mask, Y in ner_load.load_train():
    ...
    optimizer_bert.apply_gradients(
        grads_and_vars=zip(grads_bert, model.bert.variables + model.dense.variables)
    )
