    ...
    with open(input_file) as rf:
        for line in rf:
            ...
            if len(input_ids) < batch_size:
                ...
                yield tf.constant(input_ids, dtype=tf.int32), tf.constant(
                    segment_ids, dtype=tf.int32
                ), tf.constant(input_masks, dtype=tf.int32), labels
                input_ids = []
                segment_ids = []
                input_masks = []
                labels = []
...
def build_sentence_graph(batch_sentence, batch_attention_matrix, node2index, top_k=5):
    ...
    index2words = [
        {value: key for key, value in word2index.items()} for word2index in word2indexs
    ]

    batch_nodes = [
        [index2word[i] for i in range(len(index2word))]
        for k, index2word in enumerate(index2words)
    ]
    batch_value_index = [
        tf.math.top_k(attention_matrix, k=top_k)
    ...
    batch_nodes = [
        [node2index.get(w, 0) for w in nodes] for i, nodes in enumerate(batch_nodes)
    ]
    batch_edge_lists = [
        [
            [word2indexs[i][edge_pair[0]], word2indexs[i][edge_pair[1]]]
            for edge_pair in edge_list
        ]
        for i, edge_list in enumerate(batch_edge_lists)
    ]
...
bert_init_weights_from_checkpoint(
    model, model_path, param.num_hidden_layers, pooler=False
)


def build_trunk(
    model, input_file, label2index, node2index, num, mode="train", drw_attention=True
):
    nodes = []
    labels_all = []
    edge_lists = []
    ...
    count = 0
    for input_ids, segment_ids, input_masks, labels in load_inputs(
        input_file, param.maxlen, label2index, num, param.batch_size
    ):

        batch_sentence = [
            writer.convert_id_to_vocab(item) for item in input_ids.numpy()
        ]
        predict = model(
        ...
        batch_attention_matrix = [
            tf.slice(
                [predict[i]],
                [0, 1, 1],
                [-1, batch_mask_length[i] - 1, batch_mask_length[i] - 1],
            )
            for i in range(len(batch_mask_length))
        ]
        batch_sentence = [
            batch_sentence[i][1 : batch_mask_length[i]]
            for i in range(len(batch_mask_length))
        ]
        ...
        batch_attention_score = [
            tf.math.segment_sum(batch_attention_matrix[i][0], batch_word_index_list[i])
            for i in range(len(batch_word_index_list))
        ]

        batch_attention_score = [
            tf.transpose(batch_attention_score[i], [1, 0])
            for i in range(len(batch_attention_score))
        ]
        batch_attention_score = [
            tf.math.segment_sum(batch_attention_score[i], batch_word_index_list[i])
            for i in range(len(batch_word_index_list))
        ]
        batch_attention_score = [
            tf.transpose(batch_attention_score[i], [1, 0])
        ...
        batch_nodes, batch_edge_lists, batch_edge_weights = build_sentence_graph(
            batch_nsentence, batch_attention_score, node2index, top_k=5
        )

        batch_count = [
            [i + count] * len(batch_nodes[i]) for i in range(len(batch_nodes))
        ]
