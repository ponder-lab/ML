...
class BERT(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.max_position_embeddings = param.get(
            "max_position_embeddings", max_position_embeddings
        )
        self.attention_probs_dropout_prob = param.get(
    ...
    def build(self, input_shape):
        self.token_embedding = WDEmbedding(
        ...
        self.segposembedding = SegPosEmbedding(
            use_token_type=True,
    ...
    def call(self, inputs, is_training=True):
        ...
        self.embedding_output = self.segposembedding(
            self.embedding_output, token_type_ids, is_training
        )
        with tf.keras.backend.name_scope("encoder"):
            ...
            self.all_layer_attention_prob = []
            for encoder_layer in self.encoder_layers:
                layer_encode_input = layer_encode_output
                layer_encode_output, attention_prob = encoder_layer(
                    layer_encode_input, attention_mask, is_training
                )
                self.all_layer_outputs.append(layer_encode_output)
                self.all_layer_attention_prob.append(attention_prob)
            self.sequence_output = layer_encode_output
        return self

    def get_attention_matrix(self):
        return self.all_layer_attention_prob
