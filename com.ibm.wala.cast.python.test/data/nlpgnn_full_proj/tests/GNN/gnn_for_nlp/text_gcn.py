...
nodes, adjs, edge_attrs, labels, batchs, edge2index, node2index = data.build_graph(
    mode="train", p=3, k=1
)
nodes1, adjs1, edge_attrs1, labels1, batchs1, _, _ = data.build_graph(
    edge2index, node2index, mode="test", p=3
)
...
for i in range(10):
    ...
    stop_monitor = EarlyStopping(
        monitor="loss", patience=10, restore_best_weights=False
    )
    for i in range(epoch):
        ...
        for node, label, adj, edge_attr, batch in data.load(
            nodes[:-500],
            adjs[:-500],
            labels[:-500],
            edge_attrs[:-500],
                ...
                loss += (
                    tf.add_n(
                        [
                            ...
                            if "embed" and "bias" not in v.name
                        ]
                    )
                    * penalty
                )
            grads = tape.gradient(loss, model.trainable_variables)
            optimize.apply_gradients(
        ...
        print(
            "Valid: Epoch {} | Loss {:.4f} | Acc {:.4f} | Time {:.4f}".format(
                i, t_loss, t_acc, time.time() - t
            )
        )
        ...
        labs = []
        for node, label, adj, edge_attr, batch in data.load(
            nodes1, adjs1, labels1, edge_attrs1, batchs1, batch_size=32
        ):
            node, label, adj, edge_attr, batch = merge_batch_graph(
                node, label, adj, edge_attr, batch
            )
