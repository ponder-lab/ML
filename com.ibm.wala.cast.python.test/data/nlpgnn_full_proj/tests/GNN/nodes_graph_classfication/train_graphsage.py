...
for block_index in range(split):
    ...
    for i in train_bar:
        ...
        predict_train = []
        loss_train = []
        for x, y, edge_index, edge_attr, batch in dataloader.sample(
            train_data, iterator_per_epoch=6, batch_size=32, mode="train"
        ):
        ...
        report = classification_report(
            label_train, predict_train, digits=4, output_dict=True
        )
        acc = report["accuracy"]
        train_bar.set_description(
            "loss: {:.4f}, acc: {:.0f}%".format(np.mean(loss_train), 100.0 * acc)
        )
        if i != 0 and i % 50 == 0:
    ...
    predict_test = []
    for x, y, edge_index, edge_attr, batch in dataloader.sample(
        test_data, iterator_per_epoch=6, batch_size=32, mode="test"
    ):
        x, y, edge_index, edge_attr, batch = merge_batch_graph(
            x, y, edge_index, edge_attr, batch
        )
