...
class GAAE(tf.keras.Model):
    ...
    def call(self, node_embeddings, adjacency_lists, training=True):
        ...
        hidden_embeddings, reconstruct_embedding = self.model(
            node_embeddings, adjacency_lists, training
        )
        # The reconstruction loss of node features
        features_loss = tf.sqrt(
            tf.reduce_sum(
                tf.reduce_sum(tf.pow(node_embeddings - reconstruct_embedding, 2))
            )
        )
...
for p in range(epoch):
    ...
    print(
        "EPOCH {:.0f} loss {:.4f} ACC {:.4f} Time {:.4f}".format(
            p, loss_v, acc, time.time() - t
        )
    )
