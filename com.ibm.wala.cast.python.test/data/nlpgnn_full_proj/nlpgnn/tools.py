...
def albert_init_weights_from_checkpoint(
    model, checkpoint_file, num_layer=12, pooler=False
):
    ...
    if pooler:
        weights.extend(
            [loader("bert/pooler/dense/kernel"), loader("bert/pooler/dense/bias")]
        )
    ...
    for i in range(num_layer):
        encoder.extend(
            [
                ...
                loader(
                    "bert/encoder/layer_{}/attention/output/LayerNorm/gamma".format(i)
                ),
                loader(
                    "bert/encoder/layer_{}/attention/output/LayerNorm/beta".format(i)
                ),
            ]
        )
...
def uniform(shape, scale=0.05, name=None):
    initial = tf.keras.backend.random_uniform(
...
def glort(shape, name=None):
    ...
    initial = tf.keras.backend.random_uniform(
        shape, minval=-initial_range, maxval=initial_range, dtype=tf.float32
    )
    return tf.Variable(initial, name=name)
