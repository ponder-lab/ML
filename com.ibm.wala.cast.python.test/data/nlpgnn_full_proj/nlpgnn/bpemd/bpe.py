...
class BPE(object):
    def __init__(self, corpus, vocab_files, language="zh", do_low_case=True):
        ...
        if not os.path.exists(
            os.path.join(
                os.path.dirname(self.corpus), "pre_" + os.path.basename(self.corpus)
            )
        ):
            self.process_corpus()
    ...
    def wiki_bz_process(self, language):
        ...
        f = codecs.open(
            os.path.join(os.path.dirname(self.corpus), "wiki.txt"),
            "w",
            encoding="utf-8",
        )
        ...
        if language == "zh":
            for i, d in enumerate(w):
                if not re.findall("^[a-zA-Z]+:", d[0]) and not re.findall("^#", d[1]):
    ...
    def train_word2vec(self, embed_size=100, window_size=5, min_count=0, iter=10):
