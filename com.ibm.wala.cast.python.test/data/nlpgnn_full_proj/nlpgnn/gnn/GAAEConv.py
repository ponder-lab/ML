...
class GraphAttentionAutoEncoder(MessagePassing):
    ...
    def call(self, inputs, weight, transpose_b, training):
        ...
        node_embeddings = tf.linalg.matmul(
            node_embeddings, weight, transpose_b=transpose_b
        )

        node_embeddings = tf.reshape(
            node_embeddings, [node_embeddings.shape[0], self.heads, -1]
        )
