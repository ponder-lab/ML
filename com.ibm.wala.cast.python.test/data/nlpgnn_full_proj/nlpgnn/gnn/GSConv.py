...
class GraphSAGEConvolution(MessagePassing):
    ...
    def message_function(
        self,
        edge_source_states,
        edge_source,  # x_j source
        edge_target_states,
        edge_target,  # x_i target
    ...
    def propagate(self, inputs, training=None):
        ...
        messages_all_type = self._calculate_messages_all_type(
            node_embeddings, adjacency_lists, edge_weight_lists, training
        )

        # 所有关系类别中信息流动的目标节点 shape=[E]
        edge_type_to_message_targets = [
            adjacency_type_list[:, 1] for adjacency_type_list in adjacency_lists
        ]

        # 就当前节点聚合所有收到的信息，并获得新的节点状态
        new_nodes_states = self._aggregate_function(
            messages_all_type, edge_type_to_message_targets, nodes_num
        )

        return new_nodes_states

    def _calculate_messages_all_type(
        self, node_embeddings, adjacency_lists, edge_weights, training
    ):
        messages_all_type = []
        type_incoming_edges_num, type_outing_edges_num = (
            self._calculate_type_to_incoming_edges_num(node_embeddings, adjacency_lists)
        )
        for edge_type_idx, adjanceny_list_edge_type in enumerate(
            adjacency_lists
        ):  # M表示当前类的edge类型中包含的edge个数
            ...
            num_incoming_to_node_per_message = tf.gather(
                type_incoming_edges_num[edge_type_idx, :], edge_targets
            )  # message num [M], 目标每一个节点的message输入
    ...
    def call(self, inputs, training):
        ...
        if not self.concat:
            adjacency_lists = add_remain_self_loop(
                adjacency_lists, len(node_embeddings)
            )
        aggr_out = self.propagate(
            GNNInput(node_embeddings, adjacency_lists, edge_weights_lists), training
        )
        if self.concat:
