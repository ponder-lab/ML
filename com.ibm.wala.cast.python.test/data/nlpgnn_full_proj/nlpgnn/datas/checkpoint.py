...
class LoadCheckpoint(object):
    ...
    def __init__(
        self, language="zh", model="bert", parameters="base", cased=True, url=None
    ):
        """
        ...
        if model == "bert":
            if language == "zh":
                ...
                if parameters == "base":
                    ...
                    print(
                        "Other models still not support! But you could set url equal the checkpoint link."
                    )
                    url = url
        ...
        if model in ["gpt2", "gpt"]:
            for filename in self.all_files:
                ...
                if os.path.exists(os.path.join(self.gpt_base_dir, filename)):
                    if (
                        os.path.getsize(os.path.join(self.gpt_base_dir, filename))
                        != self.size
                    ):
                        print("\nFetching {} .....".format(filename))
                        self.download(
                            self.url,
                            os.path.join(self.gpt_base_dir, filename),
                            self.size,
                        )
                else:
                    print("\nFetching {} .....".format(filename))
                    self.download(
                        self.url, os.path.join(self.gpt_base_dir, filename), self.size
                    )
    ...
    def load_albert_param(self, pretraining=False):
        ...
        vocab_file = "{}/{}".format(
            "_".join(filename.split("_")[:2]), "30k-clean.vocab"
        )
        model_path = "{}/{}".format(
            "_".join(filename.split("_")[:2]), "model.ckpt-best"
        )
        spm_model_file = "{}/{}".format(
            "_".join(filename.split("_")[:2]), "30k-clean.model"
        )
