...
def glove(vocab_index_path, emb_dim):
    ...
    embeddings_index = dict(
        get_embeddings(*o.split(" "))
        for o in codecs.open(os.path.join(dirname, glove_name), encoding="utf-8")
    )
...
def fast_text(vocab_index_path, emb_dim):
    ...
    embeddings_index = dict(
        get_embeddings(*o.strip().split(" "))
        for o in codecs.open(os.path.join(dirname, fasttext_name), encoding="utf-8")
        if len(o) > 100
    )
...
def word2vec(vocab_index_path, emb_dim):
    ...
    word2vec = KeyedVectors.load_word2vec_format(
        os.path.join(dirname, word2vec_name), binary=True
    )
    embeddings_index = {}
