...
class TuckERLoader:
    ...
    def get_batch(self, er_vocab, er_vocab_pairs, batch_size=32):
        ...
        dataset = dataset.padded_batch(
            batch_size, padded_shapes={"h_r": [None], "t": [None]}, drop_remainder=True
        )
        return dataset

    def get_data_idxs(
        self, data
    ):  # data could be self.train_data self.valid_data self.test_data
        data_idxs = [
            (
                self.entity_idxs[data[i][0]],
                self.realtion_idxs[data[i][1]],
...
class GCNLoaderzero:
    ...
    def convert_2_sparse_tensor(self, sparse_matrix):
        ...
        indices = tf.constant(
            [[row, col] for row, col in zip(sparse_matrix.row, sparse_matrix.col)],
            dtype=tf.int32,
        )
        return indices
    ...
    def load(self):
        idx_features_labels = np.genfromtxt(
            "{}/{}.content".format(self.base_path, self.dataset), dtype=np.dtype(str)
        )
        ...
        edges_unordered = np.genfromtxt(
            "{}/{}.cites".format(self.base_path, self.dataset), dtype=np.int32
        )
        # [[1,2],
        #  [22,23]]
        # N*2
...
class RGCNLoader(object):
    ...
    def load_data(self):
        ...
        train_triplets = self.read_triplets(
            os.path.join(self.file_path, "train.txt"), entity2id, relation2id
        )
        valid_triplets = self.read_triplets(
            os.path.join(self.file_path, "valid.txt"), entity2id, relation2id
        )
        test_triplets = self.read_triplets(
            os.path.join(self.file_path, "test.txt"), entity2id, relation2id
        )
    ...
    def edge_normalization(self, edge_type, edge_index, num_entity, num_relation):
        ...
        """
        one_hot = tf.one_hot(
            tf.cast(edge_type, np.int32), 2 * num_relation, dtype=tf.int64
        )
        one_hot = tf.constant(one_hot.numpy())
        deg = scatter_sum(one_hot, edge_index[0], dim=0, dim_size=num_entity)
        index = edge_type + tf.keras.backend.arange(len(edge_index[0])) * (
            2 * num_relation
        )
    ...
    def generate_sampled_graph_and_labels(
        self, triplets, batch_size, split_size, num_entity, num_rels, negative_rate
    ):
        """
        ...
        # Negative sampling
        samples, labels = self.negative_sampling(
        ...
        graph_split_ids = np.random.choice(
            np.arange(batch_size), size=split_size, replace=False
        )
...
class Planetoid:
    ...
    def load(self):
        ...
        adj = tf.constant(
            [[row, col] for row, col in zip(adj.row, adj.col)], dtype=tf.int32
        )
...
class TUDataset:
    def __init__(self, name, split, data_dir="data"):
        ...
        if not os.path.exists("{}/{}".format(data_dir, name)):
            self.download()
        self.x, self.y, self.edge_index, self.edge_attr, self.num_nodes, self.batch = (
            self.read_data(self.data_dir, self.name)
        )
    ...
    def read_data(self, folder, prefix):
        ...
        batch = (
            self.read_file(folder, prefix, "graph_indicator", dtype=int) - 1
        )  # 从0开始编码
        node_attributes = node_labels = None
        if "node_attributes" in names:
            node_attributes = self.read_file(
                folder, prefix, "node_attributes", dtype=float
            )
        ...
        if "edge_attributes" in names:
            edge_attributes = self.read_file(
                folder, prefix, "edge_attributes", dtype=float
            )
    ...
    def cat(self, seq):
        ...
        seq = [
            np.expand_dims(item, -1) if len(item.shape) == 1 else item for item in seq
        ]
    ...
    def load(self, block_index=None):
        if self.split < 1:
            ...
            train_index = np.random.choice(
                np.arange(len(self.y)), size=sample_train, replace=False
            )
        ...
        trainslices, testslices, zero_start_edge_index = tudata.get_slice(
            self.x, self.y, self.edge_index, self.edge_attr, self.batch
        )

        train_data = tudata.sample_data(
            self.x,
            self.y,
            zero_start_edge_index,
            self.edge_attr,
            self.batch,
            trainslices,
        )
        test_data = tudata.sample_data(
            self.x,
            self.y,
            zero_start_edge_index,
            self.edge_attr,
    ...
    def sample_data(self, x, y, edge_index, edge_attr, batch, sample_index):
        ...
        for key, value in sample_index.items():
            if key == "x":
                ...
                batch = [
                    batch[start_end[0] : start_end[1]].tolist() for start_end in value
                ]
            elif key == "y":
                y = y[value]
            elif key == "edge_index":
                edge_index = [
                    edge_index[start_end[0] : start_end[1]].tolist()
                    for start_end in value
                ]
            elif key == "edge_attr":
                edge_attr = [
                    edge_attr[start_end[0] : start_end[1]].tolist()
                    for start_end in value
                ]
...
class Sminarog:
    def __init__(self, data="R8", data_dir="data", embedding="glove50"):
        ...
        if not os.path.exists("glove"):
            ...
            print(
                "Warning: Embedding dictionary has exists, the code will skip download {}. "
    ...
    def raw_file(self, data):
        file_name = [
            "{}-train-all-terms.txt".format(data.lower()),
            "{}-test-all-terms.txt".format(data.lower()),
        ]
        return file_name
    ...
    def map_word_to_embedding(self):
        unknown = (
            np.random.uniform(-1, 1, size=(int(self.embedding[5:]),))
            .astype(np.float32)
            .tolist()
        )
        ...
        if self.embedding == "glove50":
            ...
            raise ValueError(
                "glove_path can only in glove50, glove100, glove200, glove300 !"
            )
        ...
        with codecs.open(glove_path, encoding="utf-8") as rf:
            ...
            for line in rf:
                ...
                word2embedding[float(len(words2index))] = (
                    embedding  # 这里是为了后面能够使用
                )
    ...
    def load(self, nodes, adjs, labels, edge_attrs=None, batchs=None, batch_size=32):
        data = tf.data.Dataset.from_generator(
            self.generator(nodes, adjs, edge_attrs, labels, batchs),
    ...
    def load_graph(self, mode="train"):
        ...
        edge_attrs = np.load(
            "data/R8/{}_edge_weights.npy".format(mode), allow_pickle=True
        )
