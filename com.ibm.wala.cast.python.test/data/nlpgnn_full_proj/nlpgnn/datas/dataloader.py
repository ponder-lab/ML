        ...
        if check_exist:
            if os.path.exists(self.filename + ".tfrecords"):
                self.label_map = pickle.load(
                    open(os.path.join("Input", "label2id.pkl"), "rb")
                )
        ...
        for example in examples:
            if task.lower() == "ner":
                input_ids, segment_ids, input_mask, label_id = (
                    self._convert_single_example_ner(example, self.maxlen)
                )
            elif task.lower() == "cls":
                input_ids, segment_ids, input_mask, label_id = (
                    self._convert_single_example_cls(example, self.maxlen)
                )
