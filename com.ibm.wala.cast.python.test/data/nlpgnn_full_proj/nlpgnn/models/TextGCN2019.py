...
class TextGCN2019(tf.keras.Model):
    ...
    def call(
        self, node_embeddings, etans, edge_indexs, batchs, edge_weights, training=True
    ):
        edge_indexs = [edge_indexs]
        edge_weights = [edge_weights]
        x = tf.nn.relu(
            self.conv1(
                GNNInput(node_embeddings, edge_indexs, edge_weights, etans), training
            )
        )
    ...
    def predict(
        self, node_embeddings, etans, edge_indexs, batchs, edge_weights, training=False
    ):
