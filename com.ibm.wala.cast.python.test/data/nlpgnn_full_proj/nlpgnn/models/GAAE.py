...
class GAAELayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for layer in range(self.num_layers - 1):
            self.encoder_layers.append(
                GraphAttentionAutoEncoder(self.hidden_dim, heads=self.heads)
            )
        ...
        self.decoder_layers.append(
            GraphAttentionAutoEncoder(input_dim, heads=self.heads)
        )

    def encoder(self, node_embeddings, adjacency_lists, training):
        for layer in range(self.num_layers):
            node_embeddings = self.encoder_layers[layer](
                GNNInput(node_embeddings, adjacency_lists),
                self.weight[layer],
                False,
                training,
            )
    ...
    def decoder(self, hidden_embeddings, adjacency_lists, training):
        for layer in range(self.num_layers):
            hidden_embeddings = self.decoder_layers[layer](
                GNNInput(hidden_embeddings, adjacency_lists),
                self.weight[-(layer + 1)],
                True,
                training,
            )
    ...
    def call(self, node_embeddings, adjacency_lists, training=True):
        ...
        reconstruct_embedding = self.decoder(
            hidden_embeddings, adjacency_lists, training
        )
