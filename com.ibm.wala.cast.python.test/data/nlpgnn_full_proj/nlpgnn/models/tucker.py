...
class TuckER(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.E = tf.keras.layers.Embedding(len(loader.entities), ent_vec_dim)
        self.R = tf.keras.layers.Embedding(len(loader.relations), rel_vec_dim)
        ...
        self.W = tf.Variable(
            initial_value=tf.constant(
                np.random.uniform(
                    -1, 1, (self.rel_vec_dim, self.ent_vec_dim, self.ent_vec_dim)
                ),
                tf.float32,
            ),
            trainable=True,
