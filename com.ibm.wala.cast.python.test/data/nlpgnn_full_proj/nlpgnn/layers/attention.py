...
class MultiAttentionLayer(tf.keras.layers.Layer):
    ...
    def call(self, from_tensor, to_tensor=None, attention_mask=None, is_training=True):
        ...
        attention_score = tf.math.multiply(
            attention_score, 1.0 / math.sqrt(float(self.size_per_head))
        )
    ...
    def call(self, from_tensor, to_tensor=None, attention_mask=None, is_training=True):
        ...
        if attention_mask is not None:
            ...
            attention_mask = tf.matmul(
                broadcast_ones, tf.cast(attention_mask, tf.float32), transpose_b=True
            )
...
class GPTAttention(tf.keras.layers.Layer):
    ...
    def call(self, from_tensor, layer_past=None, is_training=True):
        ...
        distance_b = self.causal_attention_mask(
            from_length, to_length, dtype=distance.dtype
        )
        ...
        # [B,T,N,H] > [B*T,N*H]
        context_layer = tf.reshape(
            context_layer, [c_shape[0] * c_shape[1]] + [c_shape[-2] * c_shape[-1]]
        )
