...
# The windowed numpy batcher with one `if` clause on its comprehension (wala/ML#917): the filter
# drops one of the two sampled files, so the batch has ONE row, not the sample's `k`. The
# analysis cannot count what a filter admits, so the sound reading of every shape below is
# unknown rank; the asserts pin the runtime truth the static answer must not contradict.
...
assert batch_x.shape == (1, 2048), batch_x.shape
assert batch_y.shape == (1, 2048), batch_y.shape
consume_x(batch_x)
consume_y(batch_y)

raw = dataset.batch(batch_size, max_seq + 1)
assert raw.shape == (1, 2049), raw.shape
consume_raw(raw)
