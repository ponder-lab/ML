...
public class EmbeddingCall extends TensorGenerator {
  ...
  protected Set<List<Dimension<?>>> getDefaultShapes(PropagationCallGraphBuilder builder) {
    // The wala/ML#845 fallback, hoisted to the shared helper when the same gap class was swept
    // across the layer-call generators (wala/ML#855): a destructured tuple dataset element reaches
    // this parameter with exact dataflow state but no points-to evidence, and the SSA chain's
    // caller-side trace-back reaches the element's per-index generator.
